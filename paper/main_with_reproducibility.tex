...
\begin{document}
% HISTORICAL, NON-CANONICAL: Part 0 Technical Monograph is the current
% MQGT-SCF reference statement. Do not promote this paper's Born-deformation
% endpoint into the current model.
\maketitle
\begin{center}
\fbox{\parbox{0.9\linewidth}{\textbf{Historical, non-canonical paper.}
This manuscript is retained for provenance. Part 0, Technical Monograph, is
the sole current MQGT-SCF physics reference statement.}}
\end{center}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_hinv_qB_approx.png}
\caption{Approximate CMS profile likelihood scan $q(B)$ using the reported best fit and asymmetric uncertainties for the 2012--2018 combination \cite{CMS_HIG20_003}. The dashed line marks the published 95\% CL upper limit $B=0.18$.}
\label{fig:hinvq}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_hinv_gphi_posterior.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_hinv_B_vs_gphi.png}
...
All numerical results in this work are generated by an open inference harness. The repository contains the full likelihood construction, priors, sampling code, digitized experimental inputs, and reproducibility manifests. Each run is accompanied by a cryptographic hash manifest and public attestation string to ensure result integrity.

\subsection{Inference Harness}

The joint multi-channel analysis uses a Bayesian framework with the following components:

\begin{itemize}
    \item \textbf{Priors:} Uniform on $g_\phi \in [0, 0.01]$, conservative bounds on $\eta$ and $m_S$ (see config files)
    \item \textbf{Likelihood:} 
    \begin{itemize}
        \item Binomial for QRNG channel with ethically-weighted probabilities
        \item Asymmetric Gaussian approximation to CMS profile likelihood for $B(H\to\mathrm{inv})$
        \item Exclusion-based likelihood from digitized envelope for fifth-force constraints
        \item Correlated Gaussian for cosmological $(w_0, w_a)$ constraints (if included)
    \end{itemize}
    \item \textbf{Sampling:} Metropolis-Hastings MCMC with 10,000 samples, 1,000 warmup
    \item \textbf{Digitized Data:} CMS HIG-20-003 \cite{CMS_HIG20_003}, Lee et al. \cite{Lee2020ISL}
\end{itemize}

\subsection{Manifest System}

Each computational run produces:
\begin{itemize}
    \item \textbf{manifest.json:} SHA-256 hashes of all input files (configs, data), output files (samples, summaries), code versions, and timestamps
    \item \textbf{receipt.json:} Signed attestation string for result integrity
    \item \textbf{joint\_samples.csv:} MCMC samples (parameter chains)
    \item \textbf{joint\_summary.json:} Summary statistics (medians, credible intervals, effective sample sizes, convergence diagnostics)
\end{itemize}

These enable third-party verification that published results match the archived code and data. The attestation string for the published run is provided in the reproducibility package.

\subsection{Reproducibility Package}

The complete reproducibility package is archived on Zenodo (DOI: [TO BE ADDED]) and includes:
\begin{itemize}
    \item All inference harnesses (\texttt{mqgt\_qrng\_harness.py}, \texttt{mqgt\_joint\_harness.py})
    \item Manifest signing and verification tools (\texttt{mqgt\_manifest\_sign.py})
    \item Digitized experimental constraints
    \item Run outputs with manifests and receipts
    \item Complete documentation
\end{itemize}

To reproduce results:
\begin{enumerate}
    \item Install dependencies: \texttt{pip install numpy scipy matplotlib pandas numba}
    \item Run inference: \texttt{python mqgt\_joint\_harness.py run --config joint\_config\_template.json}
    \item Verify integrity: \texttt{python mqgt\_manifest\_sign.py --verify}
\end{enumerate}

See the \texttt{reproducibility/README.md} in the package for detailed instructions.

\subsection{AI-Assisted Tooling}

AI-assisted tools (including Cursor IDE) were used to accelerate code generation, refactoring, and documentation. All model choices, assumptions, priors, likelihood constructions, and interpretations were specified and verified by the author. This disclosure follows standard practice for computational physics and is analogous to disclosing use of Mathematica, GitHub Copilot, or AutoML frameworks.
